...
CHILD_MARKER = "HOME_SCREEN_MAKE_SPACE_CHILD"
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="home-screen-make-space-") as temporary:
        root = Path(temporary)
        ...
        subprocess.run(
            [environment.get("HOME_SCREEN_MAKE", "make"), "-f", str(copied / "Makefile"), "check"],
            cwd=caller,
            env=environment,
            check=True,
            timeout=180,
